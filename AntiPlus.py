...
def panel(msg):
	if msg.from_user.id in sudos or database.sismember("owners"+str(msg.chat.id), msg.from_user.id) or database.sismember("promote"+str(msg.chat.id), msg.from_user.id):
		...
		channel.add(types.InlineKeyboardButton('🌐Channel🌐', url='https://telegram.me/GetAndroid'))
...
def instagramdown(msg):
	try:
		...
		bot.send_photo(msg.chat.id, open('insta.png'), caption="Channel > [@GetAndroid]")
		os.remove('insta.png')
	except:
		print("Error!")
...
def welcome(msg):
	...
	channels.add(types.InlineKeyboardButton('🌐Channel🌐', url='https://telegram.me/GetAndroid'))
